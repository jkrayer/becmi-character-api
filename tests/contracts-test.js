...
// isNumber
describe('isNumber', () => {
  it('should only return number for integers, floats and NaN', () => {
    const { isNumber } = contracts;

    assert.strictEqual(isNumber(1), true);
    assert.strictEqual(isNumber(4.5), true);
    assert.strictEqual(isNumber(NaN), false);
    assert.strictEqual(isNumber('7'), false);
  });
});
